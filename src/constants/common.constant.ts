...
export const SectionDescription = (category: string) => {
  if (category == Category.ABOUT) {
    ...
    return '제 마음 속엔 항상 음악이 있습니다. 음악과 함께할 때만큼 가슴벅찬 느낌을 가져본 적이 없는 것 같아요.';
  } else if (category == Category.BOOK) {
    return '책이 없었다면 지금의 나보다 더 나은 사람은 되지 못했을 거예요. 언제든 책을 곁에 두면 힘이 납니다. 책을 평하는 것보다 어떤 책이든 배울 점을 찾아서 그것에 집중하는 것을 더 좋아해요.';
  } else if (category == Category.TECH) {
    return '기술에 대한 깊이가 부족해 항상 허덕입니다. 기록해두면 조금 나아지지 않을까 싶어요.';
  } else if (category == Category.QUOTE) {
    return '때론 짧은 시 한편, 누군가의 한마디, 책 속 짧은 글귀만으로 가슴이 벅찰 때가 있습니다. 그런 글을 찾아 모아두고 싶었어요.';
  } else if (category == Category.AI) {
    return 'AI 때문에 요즘 정신 못차리고 있습니다. 급변하는 세상에 동참하려면 AI 와 좀 더 친해져야 할 것 같아요.';
  } else if (category == Category.LAB) {
    return '해야할 것도 많고 알아야할 것도 많고 하고싶은 것도 많네요. 맘 속에 담아두었던 것들을 꺼내어 하나씩 시작해 봅니다.';
  }
  return '';
};
